...
MODEL_DIR = Path(__file__).resolve().parent / "biobert-maccrobat"
MODEL_NAME = "biobert_maccrobat"
DEFAULT_STRIDE = 64


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the locally fine-tuned BioBERT MACCROBAT NER model."
    )
    parser.add_argument(
        "--text",
...
def load_ner_pipeline(model_dir: Path, device: int):
    if not model_dir.is_dir():
        raise FileNotFoundError(f"BioBERT model directory not found: {model_dir}")
...
def main() -> int:
    ...
    try:
        ...
        print(f"BioBERT inference failed: {exc}", file=sys.stderr)
        return 1
