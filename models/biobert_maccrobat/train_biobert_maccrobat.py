...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fine-tune BioBERT for token classification on MACCROBAT."
    )
    parser.add_argument("--model-name", default=DEFAULT_MODEL)
    parser.add_argument("--dataset-name", default=DEFAULT_DATASET)
    parser.add_argument("--output-dir", default="outputs/biobert-maccrobat")
    ...
    parser.add_argument("--fp16", action="store_true", help="Use FP16 on a compatible CUDA GPU.")
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.validation_size < 0 or args.test_size <= 0:
        raise ValueError("Validation size must be non-negative and test size must be positive.")
    ...
    if args.fp16 and not torch.cuda.is_available():
        raise ValueError("--fp16 requires a CUDA GPU. Leave it off for MPS or CPU training.")
...
def make_splits(
    ...
    first_split = dataset.train_test_split(test_size=held_out_size, seed=seed, shuffle=True)
    relative_test_size = test_size / held_out_size
    held_out = first_split["test"].train_test_split(
...
def main() -> int:
    ...
    splits = make_splits(raw_dataset, args.validation_size, args.test_size, args.seed)
    splits["train"] = limit_split(splits["train"], args.max_train_samples)
    if "validation" in splits:
        splits["validation"] = limit_split(splits["validation"], args.max_validation_samples)
    ...
    if not tokenizer.is_fast:
        raise ValueError("A fast tokenizer is required for word-to-subword label alignment.")
    ...
    total_steps = (
        args.max_steps
        if args.max_steps > 0
        else math.ceil(steps_per_epoch * args.epochs)
    )
    warmup_steps = round(total_steps * args.warmup_ratio)
    has_validation = "validation" in tokenized_splits
    ...
    trainer = Trainer(
        ...
        eval_dataset=tokenized_splits["validation"] if has_validation else None,
